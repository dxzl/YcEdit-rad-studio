...
//---------------------------------------------------------------------------
// TYcCustomPrint constructor
//
__fastcall TYcCustomPrint::TYcCustomPrint(TYcEdit* Owner) :
...
//---------------------------------------------------------------------------
// TYcCustomPrint destructor
//
__fastcall TYcCustomPrint::~TYcCustomPrint(void)
{
}
//---------------------------------------------------------------------------
// get printer's info into member vars
//
TYcInfoDC __fastcall TYcCustomPrint::GetInfo(void)
{
  return GetInfo(Printer()->Handle);
}

TYcInfoDC __fastcall TYcCustomPrint::GetInfo(HDC hdc)
{
  TYcInfoDC info;
...
//
void __fastcall TYcCustomPrint::SetMargins(TRect margins)
{
  // set margins
  FMargins = margins;
...
void __fastcall TYcCustomPrint::SetRendDC(HDC rendDC)
{
  if (rendDC == FRendDC)
    return;
...
// and EndRender() calls
//
void TYcCustomPrint::BeginRender(int maxPages)
{
...
//
void TYcCustomPrint::EndRender(void)
{
...
//
void __fastcall TYcCustomPrint::SetShowMargins(bool showMargins)
{
...
//
CHARRANGE __fastcall TYcCustomPrint::GetSelRange(void)
{
  CHARRANGE chrg;
...
//
int __fastcall TYcCustomPrint::GetSelStartPage(void)
{
...
//
int __fastcall TYcCustomPrint::GetSelEndPage(void)
{
...
//
TYcPageOffsets __fastcall TYcCustomPrint::GetOffsetPages(CHARRANGE chrg)
{
...
//
int __fastcall TYcCustomPrint::GetPageCount(void)
{
...
::TYcPageRange __fastcall TYcCustomPrint::GetOffsetPages(int begOffset,
                                                                int endOffset)
{
  TYcPageOffsets po;
...
// actual printing)
//
bool TYcCustomPrint::PaginateTo(int page)
{
...
//
/*
int __fastcall TYcCustomPrint::GetPrinterPageCount(void)
{
...
// set the printing flag, etc., and caller should not call any TYcCustomPrint
...
// caller could screw up is by calling some TYcCustomPrint function or
...
// method of class PACKAGE TYcPreviewWindow : public TPanel. It also sets
...
bool TYcCustomPrint::RenderPage(HDC hdc, int w, int h, int page)
{
  // sanity check
  ...
  // get page offsets
  TYcPageOffsets po = FPageOffsets[page - 1];
...
void TYcCustomPrint::CalcRects(void)
{
  ...
  margRect.Left = Max((double)printableRect.Left, (double)margRect.Left);
  margRect.Top = Max((double)printableRect.Top, (double)margRect.Top);
  margRect.Right = Min((double)printableRect.Right, (double)margRect.Right);
  margRect.Bottom = Min((double)printableRect.Bottom, (double)margRect.Bottom);
...
// TYcCustomPrint actually renders to should return the rendering area for
// the TYcCustomPrint control.  this allows, say, a derived class to print
// headers and footers by returning the smaller rect actually used by
// the TYcCustomPrint control here.  this virtual function (GetRenderRect) is
...
TRect __fastcall TYcCustomPrint::GetRendRect(HDC hdc, int /*page*/)
{
...
// iHeight = 15840 twips
TRect __fastcall TYcCustomPrint::GetRendRect(HDC hdc, TRect rMargins,
                                                          int &w, int &h)
{
  TYcInfoDC info = GetInfo(hdc);
...
long int TYcCustomPrint::FormatRange(TFormatRange& formatRange, bool render)
{
...
//
void TYcCustomPrint::ClearRenderingCache(void)
{
...
//
void TYcCustomPrint::DrawMargins(HDC hdc, int /*page*/, TRect margins)
{
  // save the dc
  ::SaveDC(hdc);
...
//
void TYcCustomPrint::BeforePage(HDC hdc, int page)
{
  if (FOnBeforePage)
    (FOnBeforePage)(this, hdc, page);
}
//---------------------------------------------------------------------------
// AfterPage() - called after each page is rendered but before NewPage() is
// called
//
void TYcCustomPrint::AfterPage(HDC hdc, int page)
{
  if (FOnAfterPage)
    (FOnAfterPage)(this, hdc, page);
}
//---------------------------------------------------------------------------
// StartPrint() - called before the first page is rendered
//
void TYcCustomPrint::StartPrint(void)
{
  if (FOnStartPrint)
    (FOnStartPrint)(this);
}
...
//
void TYcCustomPrint::EndPrint(void)
{
  if (FOnEndPrint)
    (FOnEndPrint)(this);
}
...
//
TRect TYcCustomPrint::TwipsToRenderRect(TRect rect)
{
  TRect r;
...
//
TRect TYcCustomPrint::TwipsToTargetRect(HDC hdc, TRect rect)
{
  TYcInfoDC info = GetInfo(hdc);
  TRect r;
...
/*
HDC __fastcall TYcCustomPrint::CreatePrinterDC(void)
{
...
//The Rich Edit control (we a talking  about standard Windows control, not a Delphi component)
//contains built-in printing features that can be used to send formatted text to the printer or to
//paint it's content onto any canvas with minimal effort from the programmer.
//Of course, the standard Delphi TRichEdit component incapsulates this feature.
//We can use this posibility to make a fast <strong>print preview</strong> with a scaling or drawing
//Rich Text on any Delphi control.
//Drawing from a Rich Edit control to any canvas involves the use of the standard Rich Edit control
...
//The TFORMATRANGE record contains information that a rich edit control uses to format its output
//for a particular device, where
...
//This record usually is used with the EM_EXGETSEL and EM_EXSETSEL messages and includes two fields:
//cpMin and cpMax.
...
//      the range.
//long PrintRTFToBitmap(TYcEdit *ARichEdit, TBitmap *ABitmap)
//{
//  TFormatRange Range;
...
//  Range.rc.left = 0;
//  Range.rc.top = 0;
//  Range.rc.right = ABitmap->Width * 1440 / Screen->PixelsPerInch;
//  Range.rc.Bottom = ABitmap->Height * 1440 / Screen->PixelsPerInch;
//
...
//
//  // format the text
//  Result = SendMessage(ARichedit->Handle, EM_FORMATRANGE, 1, &Range);
//
...
//long PrintToCanvas(TCanvas *ACanvas, int FromChar, int ToChar, TYcEdit *ARichEdit, int AWidth, int AHeight)
//{
//  TFormatRange Range = {0};
//  //FillChar(Range, SizeOf(TFormatRange), 0);
//  Range.hdc = ACanvas->Handle;
//  Range.hdcTarget = ACanvas->Handle;
//  Range.rc.left = 0;
//  Range.rc.top = 0;
//  Range.rc.right = AWidth * 1440 / Screen->PixelsPerInch;
//  Range.rc.Bottom = AHeight * 1440 / Screen->PixelsPerInch;
...
//That is hopeless with the standard TRichedit control, because it based on the
//Windows control and have no provision to handle background bitmaps or transparency.
//In this case we can use two different bitmaps for background and drawing the
// Rich Text and after //combine them togehter.
//void Drawit()
//{
//  TBitmap *Bmp;
//  Bmp = new TBitmap;
