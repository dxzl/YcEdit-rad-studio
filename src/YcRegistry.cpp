...
void LoadFromRegistry(TRegistry& reg, String name, TPrinter& printer)
{
  ...
  String device = reg.ReadString(name + "Device");
  RegValueExists(reg, name + "Driver");
  String driver = reg.ReadString(name + "Driver");
  RegValueExists(reg, name + "Port");
  String port = reg.ReadString(name + "Port");
...
void SaveToRegistry(TRegistry& reg, String name, TStringList& list)
{
  ...
  for (int i = 0; i < list.Count; i++)
    reg.WriteString(name + "Item" + String(i), list.Strings[i]);

  // delete any entries from prior, longer list
  for (int i = list.Count; i < oldCnt; i++)
    reg.DeleteValue(name + "Item" + String(i));
}
//---------------------------------------------------------------------------
