...
//===========================================================================
//---------------------------------------------------------------------------
// TaePrint.cpp - implementation file for TaePrint.cpp
...
//---------------------------------------------------------------------------
static TYcParser Parser("mm/dd/yy", "hh:nn:ss");
static TYcPageRects PageRects;
static TYcPageRects DrawRects;
static TYcPageRects FirstPageRects;
static TYcPageRects FirstDrawRects;
//---------------------------------------------------------------------------
// TYcPrint constructor
//
__fastcall TYcPrint::TYcPrint(TYcEdit* Owner) :
  TYcCustomPrint(Owner), FHeader(new TYcHeaderText),
  FFooter(new TYcHeaderText), FFirstHeader(new TYcHeaderText),
  FFirstFooter(new TYcHeaderText), FInsideMargin(72), FBorderWidth(1),
...
// page rectangles
//
void __fastcall TYcPrint::SetAHeader(TYcHeaderText* headerFooter,
  TYcHeaderText* newHeaderFooter)
{
  *headerFooter = *newHeaderFooter;
  this->CalcRects();
}
//---------------------------------------------------------------------------
// fetch the rendering rect for the TYcRichEdit control; this overrides
// TYcCustomPrint::GetRendRect(); this function works in TWIPs
//
...
//---------------------------------------------------------------------------
// draw the margins for TYcRichEdit preview; this function works in TWIPs
//
void TYcPrint::DrawMargins(HDC hdc, int page, TRect margins)
{
  // could have different margins here, but width should be same for all pages
  TYcCustomPrint::DrawMargins(hdc, page, margins);
}
//---------------------------------------------------------------------------
// calculate the various rectangles and lines; all calculations are in TWIPs;
// the border lines will always be drawn to TYcRichEdit::FRendRect
//
// Override...
void TYcPrint::CalcRects()
{
  // Set FRendRect, inherited from TYcCustomPrint, based on FRendDC
  TYcCustomPrint::CalcRects();

// THIS MUST BE WHERE IT IS GETTING ERROR IN THE WYSIWYG HEIGHT
  CalcRectsForPage(FFirstHeader, FFirstFooter, FirstPageRects, FBorders, TargetRendRect);
  CalcRectsForPage(FHeader, FFooter, PageRects, FBorders, TargetRendRect);
}
//---------------------------------------------------------------------------
// convert the various rectangles to target units
//
void TYcPrint::CalcDrawRects(HDC hdc)
{
...
//
void TYcPrint::CalcRectsForPage(TYcHeaderText* header,
  TYcHeaderText* footer,  TYcPageRects& pageRects,
    TYcBorderLines borderLines, TRect renderRect)
{
  ...
  // margins to indent
  TYcBorderLines noBorders;
  TYcBorderLines allLeftBorders;
  ...
  // margins to indent
  TYcBorderLines allRightBorders;
...
// it finally works.
//
int TYcPrint::GetTextHeight(TYcHeaderText* header)
{
  int height = 0;
...
// GetTextHeight()).
//
int TYcPrint::DrawHeader(HDC hdc, TYcHeaderText* header,
  TRect rect)
{
  ...
  if (Printer()->Printing) {
    ...
    hOldFont = (HFONT)::SelectObject(hdc, Printer()->Canvas->Font->Handle);
    }
  else hOldFont = (HFONT)::SelectObject(hdc, font->Handle);

  // set up params for DrawTextEx()
  DRAWTEXTPARAMS dtp;
  dtp.cbSize = sizeof(DRAWTEXTPARAMS);
  dtp.iTabLength = YcEdit->TabWidth;
  dtp.iLeftMargin = 0;
  dtp.iRightMargin = 0;
  dtp.uiLengthDrawn = 0;
...
// draw border lines
//
void TYcPrint::DrawBorders(HDC hdc, TYcPageRects& pageRects,
  TYcBorderLines lines)
{
  // create empty set for comparison
  TYcBorderLines noLines;
...
//---------------------------------------------------------------------------
// override TYcCustomPrint::StartPrint() to calculate page rects and
// set unchanging parser substitution values
//
void TYcPrint::StartPrint(void)
{
  FNeedToCalcDrawRects = true;
  this->CalcRects(); // From Printer's HDC
  TYcCustomPrint::StartPrint();

  Parser.SetName(YcEdit->FileName);
  Parser.SetPages(PageCount);
  Parser.SetNow();
}
//---------------------------------------------------------------------------
// override TYcCustomPrint::AfterPage() to recalculate page rects, if
// necesssary, set the parser page value, and draw headers and footers
//
void TYcPrint::AfterPage(HDC hdc, int page)
{
  ...
  // call inherited
  TYcCustomPrint::AfterPage(hdc, page);
}
//---------------------------------------------------------------------------
// set PageStyle property
// 
void TYcPrint::SetPageStyle(TYcPageStyle& style)
{
